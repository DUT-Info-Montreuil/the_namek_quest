package universite_paris8.iut.fan.the_namek_quest.vue;

import javafx.fxml.FXML;
import javafx.scene.control.Label;
import javafx.scene.image.Image;
import javafx.scene.image.ImageView;
import javafx.scene.layout.Pane;
import javafx.scene.text.Font;
import universite_paris8.iut.fan.the_namek_quest.modele.Dende;
import universite_paris8.iut.fan.the_namek_quest.modele.GrandChef;

public class DendeVue {

    private Dende dende;
    private ImageView persoImage;
    @FXML private Pane pane;
    private GrandChef grandChef;
    private Label labelMessage = null;


    public DendeVue(Pane pane, Dende dende) {
        this.dende = dende;
        this.pane = pane;
        this.persoImage = new ImageView();
    }

    public void afficherDende() {
        if (persoImage == null) {
            Image imageDende = new Image(getClass().getResource("/universite_paris8/iut/fan/the_namek_quest/images/blanc.png").toExternalForm());
            persoImage.setImage(imageDende);
            persoImage.translateXProperty().bind(dende.getXProp());
            persoImage.translateYProperty().bind(dende.getYProp());
            pane.getChildren().add(persoImage);
        }

        else if(persoImage != null){
            pane.getChildren().remove(persoImage);
            persoImage = null;
        }
    }

    public void afficherMessageAcceuil() {
        if(dende.trunksAProximite()) {
            if (labelMessage == null) {
                labelMessage = new Label("Bonjour Trunks,\navec moi tu peux augmenter tes points de vie.\nViens devant moi et appuie sur 'p' !");
                labelMessage.setFont(new Font("Arial", 12));
                labelMessage.setPrefSize(180, 105);
                labelMessage.setWrapText(true);
                labelMessage.setStyle(
                        "-fx-background-color: white;" +
                                "-fx-border-color: black;" +
                                "-fx-background-radius: 10;" +
                                "-fx-border-radius: 10;"
                );

                labelMessage.setLayoutX(dende.getXProp().doubleValue() - 155);
                labelMessage.setLayoutY(dende.getYProp().doubleValue() - 106);
                pane.getChildren().add(labelMessage);
            }
        } else {
            if (labelMessage != null) {
                System.out.println("pas dende");
                pane.getChildren().remove(labelMessage);
                labelMessage = null;
            }
        }
    }
}
